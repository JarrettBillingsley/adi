...
If this process completes successfully, the collection of BBs is turned into a real function, and that function is then scheduled for MMU state change analysis.
...
But there are other kinds of analysis operations which can be triggered...
...
The function splitting pass takes situations like this and either splits that one function into two functions, or marks the function as **multi-entry** if its control flow graph is shaped in a way that splitting would be impossible. 

This can actually happen *before the state change analysis runs,* but it may happen *any* time after the initial first pass.
