...
use crate::ir::{ IrInst, /*IrInstKind,*/ IrSrc, IrReg };
use crate::memory::{ EA };
...
impl IrBuilder {
	...
	pub(crate) fn ircbranch_to(&mut self, ea: EA, cond: impl Into<IrSrc>, target: usize) -> usize {
		let branch_offs = self.calc_branch(self.insts.len(), target);
	...
	/// Same as above, but conditional.
	#[must_use]
	pub(crate) fn ircbranch(&mut self, ea: EA, cond: impl Into<IrSrc>) -> IrBuilderBranch {
