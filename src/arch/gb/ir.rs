...
impl IrBuilder {
	...
	fn combine_flags(&mut self, dst: IrReg) -> &mut Self {
		self
		.mov  (dst, IrConst::ZERO_8,                -1, -1)
		.ibset(dst, dst, IrConst::_8(4), REG_CF,  -1, -1, -1, -1)
		.ibset(dst, dst, IrConst::_8(5), REG_HF,  -1, -1, -1, -1)
		.ibset(dst, dst, IrConst::_8(6), REG_NF,  -1, -1, -1, -1)
		.ibset(dst, dst, IrConst::_8(7), REG_ZF,  -1, -1, -1, -1)
	}

	/// Extracts all the flag values from `src` into the flag registers.
	fn extract_flags(&mut self, src: IrReg) -> &mut Self {
		self
		.ibit(REG_CF, src, IrConst::_8(4),  -1, -1, -1)
		.ibit(REG_HF, src, IrConst::_8(5),  -1, -1, -1)
		.ibit(REG_NF, src, IrConst::_8(6),  -1, -1, -1)
		.ibit(REG_ZF, src, IrConst::_8(7),  -1, -1, -1)
	}

	/// Set the Z flag to whether or not `reg == 0`.
	fn z_(&mut self, reg: impl Into<IrReg>, regn: i8) -> &mut Self {
		let reg = reg.into();
		self.ieq(REG_ZF, reg, IrConst::_8(0),  -1, regn, -1)
	}
	...
	/// Set the Z flag to 0.
	fn z0(&mut self) -> &mut Self { self.mov(REG_ZF, IrConst::_8(0), -1, -1) }
	/// Set the N flag to 0.
	fn n0(&mut self) -> &mut Self { self.mov(REG_NF, IrConst::_8(0), -1, -1) }
	/// Set the H flag to 0.
	fn h0(&mut self) -> &mut Self { self.mov(REG_HF, IrConst::_8(0), -1, -1) }
	/// Set the C flag to 0.
	fn c0(&mut self) -> &mut Self { self.mov(REG_CF, IrConst::_8(0), -1, -1) }

	/// Set the N flag to 1.
	fn n1(&mut self) -> &mut Self { self.mov(REG_NF, IrConst::_8(1), -1, -1) }
	/// Set the H flag to 1.
	fn h1(&mut self) -> &mut Self { self.mov(REG_HF, IrConst::_8(1), -1, -1) }
	/// Set the C flag to 1.
	fn c1(&mut self) -> &mut Self { self.mov(REG_CF, IrConst::_8(1), -1, -1) }
}
...
impl IrBuilder {
	...
	fn push8(&mut self, src: impl Into<IrSrc>) -> &mut Self {
		...
		self
		.iusub(REG_SP, REG_SP, IrConst::_16(1),  -1, -1, -1)
	...
	fn pop8(&mut self, dst: impl Into<IrReg>) -> &mut Self {
		...
		self
		.load (dst,    REG_SP,                   -1, -1)
		.iuadd(REG_SP, REG_SP, IrConst::_16(1),  -1, -1, -1)
	}
...
impl IrBuilder {
	...
	fn sla(&mut self, reg: impl Into<IrReg>, regn: i8) -> &mut Self {
		let reg = reg.into();
		self
		.ibit(REG_CF, reg, IrConst::_8(7),    -1, regn, -1)
		.ishl(reg,    reg, IrConst::_8(1),  regn, regn, -1)
		.z_  (reg,                          regn)
		.n0  ()
		.h0  ()
	}
	...
	fn sra(&mut self, reg: impl Into<IrReg>, regn: i8) -> &mut Self {
		let reg = reg.into();
		self
		.ibit (REG_CF, reg, IrConst::_8(0),    -1, regn, -1)
		.isshr(reg,    reg, IrConst::_8(1),  regn, regn, -1)
		.z_   (reg,                          regn)
		.n0   ()
		.h0   ()
	}
	...
	fn srl(&mut self, reg: impl Into<IrReg>, regn: i8) -> &mut Self {
		...
		self
		.ibit (REG_CF, reg, IrConst::_8(0),    -1, regn, -1)
		.iushr(reg,    reg, IrConst::_8(1),  regn, regn, -1)
		.z_   (reg,                          regn)
		.n0   ()
		.h0   ()
	}
	...
	fn rol(&mut self, reg: impl Into<IrReg>, set_zero_flag: bool, regn: i8) -> &mut Self {
		...
		self
		.ibit(REG_CF, reg, IrConst::_8(7),    -1, regn, -1)
		.irol(reg,    reg, IrConst::_8(1),  regn, regn, -1)
		.n0  ()
		.h0  ();
	...
	fn ror(&mut self, reg: impl Into<IrReg>, set_zero_flag: bool, regn: i8) -> &mut Self {
		...
		self
		.ibit(REG_CF, reg, IrConst::_8(7),    -1, regn, -1)
		.iror(reg,    reg, IrConst::_8(1),  regn, regn, -1)
		.n0  ()
		.h0  ();
	...
	fn rolc(&mut self, reg: impl Into<IrReg>, set_zero_flag: bool, regn: i8) -> &mut Self {
		...
		.ibit (REG_CF, reg, IrConst::_8(7),           -1, regn, -1)
		.irol (reg,    reg, IrConst::_8(1),         regn, regn, -1)
		.ibset(reg,    reg, IrConst::_8(0), REG_Z,  regn, regn, -1, -1)
		.n0   ()
		.h0   ();
	...
	fn rorc(&mut self, reg: impl Into<IrReg>, set_zero_flag: bool, regn: i8) -> &mut Self {
		...
		.ibit (REG_CF, reg,   IrConst::_8(0),           -1, regn, -1)
		.iror (reg,    reg,   IrConst::_8(1),         regn, regn, -1)
		.ibset(reg,    reg,   IrConst::_8(7), REG_Z,  regn, regn, -1, -1)
		.n0   ()
		.h0   ();
	...
	fn swap(&mut self, reg: impl Into<IrReg>, regn: i8) -> &mut Self {
		let reg = reg.into();
		self
		.irol(reg, reg, IrConst::_8(4),  regn, regn, -1)
		.z_  (reg,                       regn)
		.n0  ()
		.h0  ()
		.c0  ()
	...
	fn inc_dec_hl(&mut self, plus: bool) -> &mut Self {
		if plus {
			self.iuadd(REG_HL, REG_HL, IrConst::ONE_16,  -1, -1, -1);
		} else {
			self.iusub(REG_HL, REG_HL, IrConst::ONE_16,  -1, -1, -1);
		}
...
b: &mut IrBuilder) {
	...
	match (desc.meta_op(), desc.syn_ops()) {
		...
		(DAA, []) => { // {Z*, N-, H0, C*}
			...
			.ibset(Z, Z, c(4), X,        -1, -1, -1, -1) // Z.4 = NF & CF
			.imul (Z, Z, c(-6i8 as u8),  -1, -1, -1)     // Z   = Z * -6

			// W = addition adjustment { 0x00, 0x06, 0x60, 0x66 }
			.inot (X, NF,          -1, -1)         // X   = !NF
			.iand (Y, A, c(0xF),   -1, -1, -1)     // Y   = A & 0xF
			.iugt (Y, Y, c(9),     -1, -1, -1)     // Y   = A & 0xF > 9
			.bor  (Y, Y, HF,       -1, -1, -1)     // Y   = HF | (A & 0xF > 9)
			.band (W, X, Y,        -1, -1, -1)     // W.0 = !NF & (HF | (A & 0xF > 9))
			.iugt (Y, A, c(0x99),  -1, -1, -1)     // Y   = A > 0x99
			.bor  (Y, Y, CF,       -1, -1, -1)     // Y   = CF | (A > 0x99)
			.band (Y, Y, X,        -1, -1, -1)     // Y   = !NF & (CF | (A > 0x99))
			.ibset(W, W, c(4), Y,  -1, -1, -1, -1) // W.4 = !NF & (CF | (A > 0x99))
			.imul (W, W, c(6),     -1, -1, -1)     // W   = W * 6
		...
		(LD, &[Srg(dst @ (BC | DE | HL)), Op]) => { // no flag changes
			...
			b
			.mov(REG_WZ, IrConst::_16(val as u16),  -1,  0)
		...
		(LD, &[IndOp, Srg(SP)]) => { // no flag changes
			...
			.store(IrConst::_16(dst.0 as u16), REG_Z,   0, -1)
			...
			.store(IrConst::_16((dst.0 + 1) as u16), REG_W,  -1, -1);
		}
