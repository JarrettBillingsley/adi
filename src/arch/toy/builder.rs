...
impl ToyBuilder {
	...
	pub fn org(&mut self, addr: Offs) {
		assert!(addr >= self.bytes.len() as Offs, "can only move origin up");
		assert!(addr <= 0x7FFF, "invalid address {:X} (valid addresses 0..=0x7FFF)", addr);
		self.bytes.resize(to_usize(addr), 0);
	}

	pub fn append(&mut self, bytes: &[u8]) -> Offs {
	...
	pub fn branch_here(&mut self, from: Offs) -> Offs {
		assert!(from < self.bytes.len() as Offs, "bad source address {:X}", from);
		let opc = self.bytes[to_usize(from)];
		...
		self.bytes[to_usize(from + 1)] = calc_branch_offset(from, self.bytes.len() as Offs);
		self.bytes.len() as Offs
	}

	pub fn jump_here(&mut self, from: Offs) -> Offs {
		assert!(from < self.bytes.len() as Offs, "bad source address {:X}", from);
		let opc = self.bytes[to_usize(from)];
		...
		let target = encode_16bit_addr(self.bytes.len() as Offs);
		self.bytes[to_usize(from + 1)] = target[0];
		self.bytes[to_usize(from + 2)] = target[1];
		self.bytes.len() as Offs
	}

	pub fn mov(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::MOV_RR, dst), encode_r1_8bit(src)])
	}

	pub fn movi(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::MOV_RI8, dst), src.into()])
	}

	pub fn add(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::ADD_RR, dst), encode_r1_8bit(src)])
	}

	pub fn addi(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::ADD_RI8, dst), src.into()])
	}

	pub fn adc(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::ADC_RR, dst), encode_r1_8bit(src)])
	}

	pub fn adci(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::ADC_RI8, dst), src.into()])
	}

	pub fn sub(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::SUB_RR, dst), encode_r1_8bit(src)])
	}

	pub fn subi(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::SUB_RI8, dst), src.into()])
	}

	pub fn sbc(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::SBC_RR, dst), encode_r1_8bit(src)])
	}

	pub fn sbci(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::SBC_RI8, dst), src.into()])
	}

	pub fn and(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::AND_RR, dst), encode_r1_8bit(src)])
	}

	pub fn andi(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::AND_RI8, dst), src.into()])
	}

	pub fn or(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::OR_RR, dst), encode_r1_8bit(src)])
	}

	pub fn ori(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::OR_RI8, dst), src.into()])
	}

	pub fn xor(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::XOR_RR, dst), encode_r1_8bit(src)])
	}

	pub fn xori(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::XOR_RI8, dst), src.into()])
	}

	pub fn not(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::NOT_RR, dst), encode_r1_8bit(src)])
	}

	pub fn noti(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::NOT_RI8, dst), src.into()])
	}

	pub fn cmp(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::CMP_RR, dst), encode_r1_8bit(src)])
	}

	pub fn cmpi(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::CMP_RI8, dst), src.into()])
	}

	pub fn cmc(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::CMC_RR, dst), encode_r1_8bit(src)])
	}

	pub fn cmci(&mut self, dst: Reg, src: impl Into<u8>) -> Offs {
		self.append(&[encode_opr0(Opcode::CMC_RI8, dst), src.into()])
	}

	pub fn blt(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::BLT_S8), 0])
	}

	pub fn blt_to(&mut self, to: Offs) -> Offs {
		self.append(&[encode_op(Opcode::BLT_S8), calc_branch_offset(self.bytes.len() as Offs, to)])
	}

	pub fn ble(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::BLE_S8), 0])
	}

	pub fn ble_to(&mut self, to: Offs) -> Offs {
		self.append(&[encode_op(Opcode::BLE_S8), calc_branch_offset(self.bytes.len() as Offs, to)])
	}

	pub fn beq(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::BEQ_S8), 0])
	}

	pub fn beq_to(&mut self, to: Offs) -> Offs {
		self.append(&[encode_op(Opcode::BEQ_S8), calc_branch_offset(self.bytes.len() as Offs, to)])
	}

	pub fn bne(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::BNE_S8), 0])
	}

	pub fn bne_to(&mut self, to: Offs) -> Offs {
		self.append(&[encode_op(Opcode::BNE_S8), calc_branch_offset(self.bytes.len() as Offs, to)])
	}

	pub fn jmp(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::JMP_I16), 0, 0])
	}

	pub fn jmp_to(&mut self, target: Offs) -> Offs {
		let target = encode_16bit_addr(target);
		self.append(&[encode_op(Opcode::JMP_I16), target[0], target[1]])
	}

	pub fn jmpi(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::JMPI_IMPDC)])
	}

	pub fn call(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::CALL_I16), 0, 0])
	}

	pub fn cali(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::CALI_IMPDC)])
	}

	pub fn calz(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::CALZ_I16), 0, 0])
	}

	pub fn call_to(&mut self, target: Offs) -> Offs {
		let target = encode_16bit_addr(target);
		self.append(&[encode_op(Opcode::CALL_I16), target[0], target[1]])
	}

	pub fn calz_to(&mut self, target: Offs) -> Offs {
		let target = encode_16bit_addr(target);
		self.append(&[encode_op(Opcode::CALZ_I16), target[0], target[1]])
	}

	pub fn ret(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::RET_IMP)])
	}

	pub fn retz(&mut self) -> Offs {
		self.append(&[encode_op(Opcode::RETZ_IMP)])
	}

	pub fn ld(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::LD_RR, dst), encode_r1_any(src)])
	}

	pub fn ldi(&mut self, dst: Reg, addr: u16) -> Offs {
		let addr = encode_16bit_addr(addr as Offs);
		self.append(&[encode_opr0(Opcode::LD_RI16, dst), addr[0], addr[1]])
	}

	pub fn st(&mut self, dst: Reg, src: Reg) -> Offs {
		self.append(&[encode_opr0(Opcode::ST_RR, dst), encode_r1_any(src)])
	}

	pub fn sti(&mut self, dst: Reg, addr: u16) -> Offs {
		let addr = encode_16bit_addr(addr as Offs);
