...
impl InstDesc {
	...
	fn get_operand(&self, i: &Instruction, b: &mut IrBuilder) -> IrSrc {
		...
		match self.addr_mode {
			...
			ZPX | ZPY => {
				...
				// tmp2 = reg + disp
				b.iuadd(REG_TMP2, reg_to_ir_reg(reg), (IrConst::_8(disp as u8), 0));
				REG_TMP2.into()
			}
			...
			ABX | ABY => {
				...
				// tmp16 = zxt(reg)
				// tmp16 += disp
				b.izxt( REG_TMP16, reg_to_ir_reg(reg));
				b.iuadd(REG_TMP16, REG_TMP16, (IrConst::_16(disp as u16), 0));
				REG_TMP16.into()
			}
			...
			IND => {
				...
				if (va & 0xFF) == 0xFF {
					...
					b.ipair(REG_TMP16, REG_TMP1, REG_TMP2);
				} else {
					// ez mode
			...
			IZX => {
				...
				b.iuadd(REG_TMP2, REG_X, (IrConst::_8(disp as u8), 0));
				...
				b.iuadd(REG_TMP1, REG_TMP2, IrConst::ONE_8);
			...
			IZY => {
				...
				// tmp16_2 = zxt(Y)
				// tmp16 = tmp16 + tmp16_2
				b.izxt( REG_TMP16_2, REG_Y);
				b.iuadd(REG_TMP16, REG_TMP16, REG_TMP16_2);
				REG_TMP16.into()
			}
	...
	fn return_(&self, b: &mut IrBuilder, add_1: bool) {
		...
		if add_1 {
			b.iuadd(REG_TMP16, REG_TMP16, IrConst::ONE_16);
		}
	...
	fn push_flags(&self, b: &mut IrBuilder) {
		...
		b.ibset(REG_TMP1, REG_TMP1, IrConst::_8(0), REG_CF);
		b.ibset(REG_TMP1, REG_TMP1, IrConst::_8(1), REG_ZF);
		b.ibset(REG_TMP1, REG_TMP1, IrConst::_8(2), REG_IF);
		b.ibset(REG_TMP1, REG_TMP1, IrConst::_8(3), REG_DF);
		b.ibset(REG_TMP1, REG_TMP1, IrConst::_8(6), REG_VF);
		b.ibset(REG_TMP1, REG_TMP1, IrConst::_8(7), REG_NF);
	...
	fn pop_flags(&self, b: &mut IrBuilder) {
		self.pop8(REG_TMP1, b);
		b.ibit(REG_CF, REG_TMP1, IrConst::_8(0));
		b.ibit(REG_ZF, REG_TMP1, IrConst::_8(1));
		b.ibit(REG_IF, REG_TMP1, IrConst::_8(2));
		b.ibit(REG_DF, REG_TMP1, IrConst::_8(3));
		b.ibit(REG_VF, REG_TMP1, IrConst::_8(6));
		b.ibit(REG_NF, REG_TMP1, IrConst::_8(7));
	}

	/// Do a comparison and set flags according to the result.
	fn cmp(&self, src1: IrReg, src2: impl Into<BuildReg>, b: &mut IrBuilder) {
		let src2 = src2.into();
		b.iusub(REG_TMP1, src1, src2);
		// CF = not(src1 <u src2)
		b.iult (REG_CF,   src1, src2);
	...
	fn add_(&self, src: impl Into<IrSrc>, srcn: i8, b: &mut IrBuilder) {
		let src = src.into();
		b.iscarryc(REG_VF, REG_A, src, REG_CF);
		b.iucarryc(REG_CF, REG_A, src, REG_CF);
		b.iuaddc  (REG_A,  REG_A, (src, srcn), REG_CF);
		self.set_nz(REG_A, b);
	}

	/// Sets the Negative and Zero flags based on the value of the given `reg`.
	fn set_nz(&self, reg: IrReg, b: &mut IrBuilder) {
		b.islt(REG_NF, reg, IrConst::ZERO_8);
		b.ieq (REG_ZF, reg, IrConst::ZERO_8);
	}

	/// Sets the Carry flag to 1 if MSB of `reg` is 1.
	fn set_c(&self, reg: IrReg, b: &mut IrBuilder) {
		b.islt(REG_CF, reg, IrConst::ZERO_8);
	}
	...
	pub(super) fn build_ir(&self, i: &Instruction, term: Option<&BBTerm>, b: &mut IrBuilder) {
		...
		match self.meta_op {
			...
			DEC => { // NZ
				...
				b.iusub(REG_TMP1, REG_TMP1, IrConst::ONE_8);
				b.store((addr, 0), REG_TMP1);
				self.set_nz(REG_TMP1, b);
			}
			DEX => { // NZ
				b.iusub(REG_X, REG_X, IrConst::ONE_8);
				self.set_nz(REG_X, b);
			}
			DEY => { // NZ
				b.iusub(REG_Y, REG_Y, IrConst::ONE_8);
				self.set_nz(REG_Y, b);
			}
			INC => { // NZ
				let addr = self.get_operand(i, b);
				b.load (REG_TMP1, (addr, 0));
				b.iuadd(REG_TMP1, REG_TMP1, IrConst::ONE_8);
				b.store((addr, 0), REG_TMP1);
				self.set_nz(REG_TMP1, b);
			}
			INX => { // NZ
				b.iuadd(REG_X, REG_X, IrConst::ONE_8);
				self.set_nz(REG_X, b);
			}
			INY => { // NZ
				b.iuadd(REG_Y, REG_Y, IrConst::ONE_8);
				self.set_nz(REG_Y, b);
			}
			...
			BIT => { // NZV (NF = mem.7, VF = mem.6, ZF = whether A&op is 0)
				...
				b.ieq (REG_ZF,   REG_TMP1, IrConst::ZERO_8);
			}
			...
			ASL => { // NZC
				...
				b.ishl(REG_TMP1, REG_TMP1, IrConst::ONE_8);
				self.set_nz(REG_TMP1, b);
			...
			ASLA => { // NZC
				self.set_c(REG_A, b);
				b.ishl(REG_A, REG_A, IrConst::ONE_8);
				self.set_nz(REG_A, b);
			}
			LSR => { // NZC (NF = 0, hardcoded)
				...
				b.iushr(REG_TMP1, REG_TMP1, IrConst::ONE_8);
				self.set_nz(REG_TMP1, b);
			...
			LSRA => { // NZC (NF = 0, hardcoded)
				...
				b.iushr(REG_A,  REG_A, IrConst::ONE_8);
				self.set_nz(REG_A, b);
			}
			ROL => { // NZC
				let addr = self.get_operand(i, b);
				...
				b.ishl(REG_TMP1, REG_TMP1, IrConst::ONE_8); // a = a << 1
			...
			ROLA => { // NZC
				...
				b.ishl(REG_A, REG_A, IrConst::ONE_8); // a = a << 1
			...
			ROR => { // NZC
				...
				b.iushr (REG_TMP1, REG_TMP1, IrConst::ONE_8); // a = a << 1
				// TODO: do this with bit set/get IR instructions
				b.ishl  (REG_TMP2, REG_TMP2, IrConst::_8(7)); // tmp2 <<= 7
				b.ior   (REG_TMP1, REG_TMP1, REG_TMP2); // a = a | tmp2
				self.set_nz(REG_TMP1, b);
			...
			RORA  => { // NZC
				...
				b.iushr (REG_A,    REG_A,    IrConst::ONE_8); // a = a << 1
				// TODO: do this with bit set/get IR instructions
				b.ishl  (REG_TMP2, REG_TMP2, IrConst::_8(7)); // tmp2 <<= 7
				b.ior   (REG_A,    REG_A,    REG_TMP2); // a = a | tmp2
				self.set_nz(REG_A, b);
