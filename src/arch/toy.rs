...
-> (usize, Option<VA>) {
	...
	match desc.addr_mode {
		...
		S8 => {
			let target = VA(((va.0 as isize) + (img[1] as i8 as isize) + 2) as Offs);
			ops[0] = Operand::Mem(target, MemAccess::Target);
			(1, Some(target))
		}
		I16 => {
			let target = VA(u16::from_le_bytes(img[1..].try_into().unwrap()) as Offs);
			ops[0] = Operand::Mem(target, MemAccess::Target);
			(1, Some(target))
		}
		RI16 => {
			let addr = VA(u16::from_le_bytes(img[1..].try_into().unwrap()) as Offs);
			ops[0] = opcode_reg;
